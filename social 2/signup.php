...
if (!isset($_SESSION['signed_in']) && !isset($_POST['submit'])){
    ...
    if ($user_name !== $row['user_name']){
        if ($user_pass == $user_conf){
            mysql_query("INSERT INTO users (user_name, user_pass, user_date, user_email) VALUES ('$user_name', '$user_pass', '$date', '$user_email')");
            echo 'Succesfully registered. You can now <a href="signin.php">sign in</a> and start adding mods and servers :D :)';
        } else {
