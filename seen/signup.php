...
echo '<script src="includes/js/user_check.js" type="text/javascript"></script>';
echo '<h2>Sign up</h2>';
            ...
            if ($user_name !== $user_name_row){
                if ($user_pass == $user_conf){
                    if ($user_email !== $user_email_row){
                        ...
                        $message = 'To activate your account, please click on the link below:\n\n http://www.TylerBridger.com/signin.php?activate=true&key='.$activation;
                        mail($user_email, 'Registration Confirmation', $message, 'From: no-reply@tylerbridger.com');
