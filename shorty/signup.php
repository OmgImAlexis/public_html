...
if (!isset($_SESSION['signed_in']) && !isset($_POST['submit'])){
    ...
    $user_api_key = hash('whirlpool', 'randomstringforsaltkey'.$user_name);
    ...
    if ($user_name !== $row['user_name']){
        if ($user_pass == $user_conf){
            mysql_query("INSERT INTO users (user_name, user_pass, user_date, user_email, user_api_key) VALUES ('$user_name', '$user_pass', '$date', '$user_email', '$user_api_key')");
            echo 'Succesfully registered. You can now <a href="signin.php">sign in</a> and start shortening! :)';
        } else {
